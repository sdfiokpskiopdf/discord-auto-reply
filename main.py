...
class MyClient(discord.Client):
    ...
    async def on_message(self, message):
        ...
        if (
            ...
            and self.ignore in message.content
        ):
            return
...
class MainApplication(tk.Frame):
    def __init__(self, master=None, **kwargs):
        ...
        tk.Label(self, relief="sunken", text="Token:", anchor="w").grid(
            row=0, column=0, sticky="we"
        )
        tk.Entry(self, relief="sunken", textvariable=self.tokenStorage).grid(
            row=0, column=1, sticky="we"
        )
        tk.Label(self, relief="sunken", text="Start:", anchor="w").grid(
            row=1, column=0, sticky="we"
        )
        tk.Label(self, relief="sunken", text="End:", anchor="w").grid(
            row=1, column=1, sticky="we"
        )
        tk.OptionMenu(self, self.startStorage, *self.times).grid(
            row=2, column=0, sticky="we"
        )
        tk.OptionMenu(self, self.endStorage, *self.times).grid(
        ...
        tk.Checkbutton(
            ...
            text="Ignore:",
            relief="sunken",
            variable=self.ignoreStorage,
            command=lambda: self.activate_entry(),
        ).grid(row=3, column=0, sticky="we")
        self.ignore = tk.Entry(
            self, relief="sunken", textvariable=self.ignoreTextStorage, state="disabled"
        )
        self.ignore.grid(row=3, column=1, sticky="we")
        tk.Label(self, relief="sunken", text="Message:", anchor="w").grid(
            row=4, column=0, sticky="we"
        )
        tk.Entry(self, relief="sunken", textvariable=self.messageStorage).grid(
            row=4, column=1, sticky="we"
        )
        tk.Button(self, text="Start", command=lambda: self.control_thread()).grid(
            row=5, column=0, sticky="we"
        )
        tk.Button(self, text="End", command=lambda: self.stop_thread()).grid(
            row=5, column=1, sticky="we"
        )
        tk.Button(
            self, textvariable=self.buttonStorage, command=lambda: self.create_window()
        ).grid(row=6, column=0, columnspan=2, sticky="we")
        tk.Button(
            self, textvariable=self.pauseStorage, command=lambda: self.interrupt()
    ...
    def create_window(self):
        ...
        if self.window_open:
            ...
            if len(self.history_data) == 0:
                tk.Label(self.window, text="No history to show yet.").place(
                    relx=0.2, rely=0.4
                )
                return None
            ...
            for item in self.history_data:
                ...
                tk.Label(
                    self.window, relief="sunken", text=item["time"], anchor="w"
                ).grid(row=i, column=0, sticky="we")
                tk.Label(
                    self.window, relief="sunken", text=item["author"], anchor="w"
                ).grid(row=i, column=1, sticky="we")
                tk.Label(
                    self.window, relief="sunken", text=item["content"], anchor="w"
    ...
    def stop_thread(self, verify=True):
        if verify:
            result = self.question_popup(
                "Exit", "Are you sure you want to end the program?"
            )
            if result == "yes":
                sys.exit()
        else:
